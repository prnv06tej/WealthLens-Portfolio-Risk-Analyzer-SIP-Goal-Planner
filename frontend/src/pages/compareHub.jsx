...
export default function CompareHub() {
  ...
  const handleCompare = async () => {
    ...
    try {
      const response = await API.get(
        `/funds/overlap?fundA=${fundA}&fundB=${fundB}`,
      );
  ...
  const getRiskStatus = (percentage) => {
    ...
    if (num < 20)
      return {
        label: "Excellent Diversification",
        color: "#16a34a",
        bg: "#f0fdf4",
      };
    if (num < 50)
      return {
        label: "Moderate Overlap (Monitor Closely)",
        color: "#d97706",
        bg: "#fffbeb",
      };
    return {
      label: "High Overlap Danger (Redundant Positions)",
      color: "#dc2626",
      bg: "#fef2f2",
    };
  };
  ...
  return (
    ...
    >
      <div style={{ marginBottom: "35px" }}>
        <h1 style={{ fontSize: "2.2rem", fontWeight: "700", color: "#0f172a" }}>
          Portfolio Overlap Analyzer
        </h1>
        <p style={{ color: "#64748b", marginTop: "6px" }}>
          Cross-reference two fund profiles to protect your investments from
          hidden stock concentration risk.
        </p>
      </div>
      ...
      {analysisResult && (
        <div style={{ marginTop: "40px" }}>
          ...
          <div
            style={{
              ...getRiskStatus(analysisResult.overlapPercentage),
              display: "flex",
              alignItems: "center",
              gap: "20px",
              padding: "25px",
          ...
          >
            {(analysisResult.overlapPercentage || 0) >= 50 ? (
            ...
            <div>
              ...
              >
                {analysisResult.overlapPercentage || 0}%
              </h2>
              <p style={{ fontWeight: "600", fontSize: "1.1rem" }}>
                {getRiskStatus(analysisResult.overlapPercentage).label}
              </p>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              >
                Intersecting Stock Holdings (
                {analysisResult.commonStocks?.length || 0})
              </div>
              <div style={{ maxHeight: "400px", overflowY: "auto" }}>
                {/*  FIXED: Safe defensive checks for missing arrays */}
                {!analysisResult.commonStocks ||
                analysisResult.commonStocks.length === 0 ? (
                  <p
                    style={{
                      padding: "20px",
                  ...
                  >
                    Zero common stock conflicts found between these selections.
                  </p>
                ) : (
                  <table
                    style={{
                  ...
                  >
                    ...
                    <tbody>
                      {/* 🌟 FIXED: Added optional chaining block */}
            ...
            >
              ...
              >
                ...
                >
                  Portfolio overlap measures the true replication between two
                  funds. For every stock held in common, the algorithm tracks
                  the <strong>lesser allocation percentage</strong> to pinpoint
                  the exact duplicate exposure in your combined strategy.
                </p>
                ...
                >
                  ...
                  >
                    Mathematical Model
                     ...
                     Overlap % = Σ<sub>i</sub> min(W<sub>A,i</sub>, W<sub>B,i</sub>)
                </div>
              </div>
            </div>
          </div>
